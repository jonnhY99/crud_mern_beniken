import React from 'react';

const LayoutFooter = () => {
  return (
    <footer className="bg-red-700 text-white">
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          {/* Marca */}
          <div className="md:col-span-2">
            <h3 className="text-lg font-semibold">Carnes Beniken</h3>
            <p className="mt-4 text-sm">
              Tradición, confianza y excelencia al servicio de nuestros clientes. 
                  Ahora también con plataforma en línea para agilizar tus compras..
            </p>
          </div>

          {/* Contacto */}
          <div id="contact">
            <h3 className="text-sm font-semibold tracking-wider uppercase">
              Contacto
            </h3>
            <ul className="mt-4 space-y-2">
              <li>
                <a
                  href="mailto:support@beniken.com"
                  className="hover:underline text-sm"
                >
                  support@beniken.com
                </a>
              </li>
              <li>
                <a
                  href="tel:+56922222222"
                  className="hover:underline text-sm"
                >
                  +56 9 2222 2222
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="hover:underline text-sm"
                >
                  Mercado Matadero Franklin, local 382, Santiago.
                </a>
              </li>
            </ul>
          </div>

          {/* Redes Sociales */}
          <div>
            <h3 className="text-sm font-semibold tracking-wider uppercase">
              Redes Sociales
            </h3>
            <div className="mt-4 flex space-x-6">
              {/* Facebook */}
              <a
                href="https://www.facebook.com/luisulises.cofrepezo"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-gray-200"
              >
                <span className="sr-only">Facebook</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M22 12c0-5.523-4.477-10-10-10S2 
                       6.477 2 12c0 4.991 3.657 9.128 
                       8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 
                       1.492-3.89 3.777-3.89 1.094 0 2.238.195 
                       2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 
                       1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 
                       21.128 22 16.991 22 12z"
                  />
                </svg>
              </a>

              {/* Instagram */}
              <a
                href="https://www.instagram.com/luisulisescofre/"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-gray-200"
              >
                <span className="sr-only">Instagram</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M12.315 2c2.43 0 2.784.013 
                       3.808.06 1.064.049 1.791.218 
                       2.427.465a4.902 4.902 0 011.772 
                       1.153 4.902 4.902 0 011.153 
                       1.772c.247.636.416 1.363.465 
                       2.427.048 1.067.06 1.407.06 
                       4.123v.08c0 2.643-.012 2.987-.06 
                       4.043-.049 1.064-.218 1.791-.465 
                       2.427a4.902 4.902 0 01-1.153 
                       1.772 4.902 4.902 0 01-1.772 
                       1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 
                       0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 
                       4.902 0 01-1.772-1.153 4.902 
                       4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 
                       4.902 0 011.153-1.772A4.902 
                       4.902 0 015.45 2.525c.636-.247 
                       1.363-.416 2.427-.465C8.901 
                       2.013 9.256 2 11.685 2h.63zM12 
                       6.865a5.135 5.135 0 110 10.27 
                       5.135 5.135 0 010-10.27zm0 
                       1.802a3.333 3.333 0 100 6.666 
                       3.333 3.333 0 000-6.666zm5.338-3.205a1.2 
                       1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                  />
                </svg>
              </a>

              {/* Twitter */}
              <a
                href="https://twitter.com/TU_USUARIO"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-gray-200"
              >
                <span className="sr-only">Twitter</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 
                           11.675-11.675 0-.178 0-.355-.012-.53A8.348 
                           8.348 0 0022 5.92a8.19 8.19 0 
                           01-2.357.646 4.118 4.118 0 
                           001.804-2.27 8.224 8.224 0 
                           01-2.605.996 4.107 4.107 0 
                           00-6.993 3.743 11.65 11.65 0 
                           01-8.457-4.287 4.106 4.106 
                           0 001.27 5.477A4.072 4.072 
                           0 012.8 9.713v.052a4.105 
                           4.105 0 003.292 4.022 4.095 
                           4.095 0 01-1.853.07 4.108 
                           4.108 0 003.834 2.85A8.233 
                           8.233 0 012 18.407a11.616 
                           11.616 0 006.29 1.84" />
                </svg>
              </a>

              {/* TikTok */}
              <a href="https://www.tiktok.com/@ulicofre1" className="hover:text-gray-200">
  <span className="sr-only">TikTok</span>
  <svg
    className="h-6 w-6"
    fill="currentColor"
    viewBox="0 0 24 24"
    aria-hidden="true"
  >
    <path d="M12 2c1.104 0 2 .896 2 2v1.053c1.568 0 3.018.462 4.24 1.257a6.903 6.903 0 001.647 3.414A7.032 7.032 0 0022 10.5V14a2 2 0 01-2 2c-1.104 0-2-.896-2-2v-.5a9.953 9.953 0 01-3-.5V17a7 7 0 11-7-7c.338 0 .668.03.99.088V12a3.99 3.99 0 00-1.99-.526A4 4 0 0010 19a4 4 0 004-4V4a2 2 0 012-2h-4z" />
  </svg>
</a>


            </div>
          </div>
        </div>

        {/* Footer Bottom */}
        <div className="mt-8 border-t border-red-600 pt-8 md:flex md:items-center md:justify-between">
          <p className="text-sm">
            © 2025 Carnicería Beniken. Todos los derechos reservados.
          </p>
          <div className="mt-4 md:mt-0">
            <a href="#" className="hover:underline text-sm mr-4">
              Términos
            </a>
            <a href="#" className="hover:underline text-sm mr-4">
              Privacidad
            </a>
            <a href="#" className="hover:underline text-sm">
              FAQ
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default LayoutFooter;
